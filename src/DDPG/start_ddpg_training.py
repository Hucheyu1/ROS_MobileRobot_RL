...
if __name__ == '__main__':
    ...
    stage_name = "stage_none_nowaypoint"
    ...
    for ep in range(resume_epoch, nepisodes):
        ...
        for step in range(nsteps):
            ...
            if learning:
                action = ddpg_trainer.act(state, step_counter, add_noise=True)
            else:
                action = ddpg_trainer.act(state, step_counter, add_noise=False)
            ...
            if learning:
                ...
                if len(ddpg_trainer.memory) > batch_size :
                    ...
                    if  ep > 300 and ep <= 600 and step_counter % 10 == 0:
                        ddpg_trainer.learn(step_counter)                   
            if not done:
            ...
            if done:
                ...
                if learning:
                    utils.record_data(data, result_outdir,"ddpg_training_trajectory_2")
                else:
                    utils.record_data(data, result_outdir,"ddpg_test_no_move")
