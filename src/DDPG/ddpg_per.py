...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        action = self.forward_model(state)
...
class Agent:
    ...
    def __init__(self, state_size, action_size, hidden_size, actor_learning_rate, critic_learning_rate, batch_size,
                 buffer_size, discount_factor, softupdate_coefficient, max_lin_vel, max_ang_vel, prioritized):
        ...
        if not self.prioritized:
            ...
            self.prioritized_replay_alpha = 0
            self.prioritized_replay_beta0 = 0
            self.epsilon = 1e-6
            self.memory = PER_buffer.PrioritizedReplayBuffer(buffer_size, self.prioritized_replay_alpha)
    ...
    def learn(self , ep):
        ...
        """
        if self.prioritized:
            beta_schedule = util.LinearSchedule(100000, initial_p=self.prioritized_replay_beta0, final_p=1.0)
            beta_value = beta_schedule.value(ep)
            states, actions, rewards, next_states, dones, weights, batch_idxes = self.memory.sample(self.batch_size, beta_value)
