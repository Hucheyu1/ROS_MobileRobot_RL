...
class Agent:
    ...
    def learn(self, step, decay_step):
        """
        ...
        # self.noise_std = self.noise.get_sigma(decay_step)
        # noise_v = torch.normal(torch.zeros(next_action[:,0].size()), self.noise_std).to(device)
        # noise_v = torch.clamp(noise_v, -self.noise_clip, self.noise_clip)
        # noise_w = torch.normal(torch.zeros(next_action[:,1].size()), self.noise_std*5).to(device)
        # noise_w = torch.clamp(noise_w, -self.noise_clip*4, self.noise_clip*4)
        # next_action[: , 0] += noise_v
        # next_action[: , 1] += noise_w
